...
from dadou_utils_ros.utils_static import LOGGING_TEST_FILE_NAME, LOGGING_LAPTOP_TEST_FILE_NAME, ID, LOGGING_TEST_FILE
...
class TestChatgpt(unittest.TestCase):
    ...
    logging.config.dictConfig(LoggingConf.get(config[LOGGING_TEST_FILE], "test_chat_gpt"))
    ...
    def test_text_photo(self):
        camera = AICamera()
        camera.take_photo()
        response = self.robot_dialog.chatgpt_request("décris la photo ?", new_photo=True)
        logging.info(response)
